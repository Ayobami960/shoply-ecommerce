...
async function fulfillCheckoutSession(
    ...
    return await db.transaction(async (tx) => {
        ...
        // if it deos not exist then return false
        if (!session) return false;

        // then if it exist then we can create an order, servies we be paid and we put all the details
        const [order] = await tx
            ...
            // if the session it success the order failed since it under a transaction both of then will failed so it ether all success or failed
        ...
        // we detete the session if user do not need it
